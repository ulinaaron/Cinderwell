...
registerBlockType( metadata.name, {
    // Legacy block: hidden from the inserter, kept so existing content renders.
    // Converts to Columns with body blocks for the header, each column, and footnote.
    transforms: {
        to: [
            {
                type: 'block',
                blocks: [ 'cinderwell/columns' ],
                transform: ( attributes ) => {
                    const body = ( attrs ) => createBlock( 'cinderwell/body', { showHeading: false, showBody: true, ...attrs } );
                    const header = attributes.showEyebrow && attributes.eyebrow || attributes.showHeading && attributes.heading
                        ? [ createBlock( 'cinderwell/body', {
                        ...
                        : [];
                    const footnote = attributes.showFootnote && attributes.footnote
                        ? [ createBlock( 'cinderwell/body', { showBody: false, showHeading: false, showFootnote: true, footnote: attributes.footnote, background: attributes.background, width: attributes.width } ) ]
                        : [];
                    const columns = createBlock(
                        'cinderwell/columns',
                        { background: attributes.background, width: attributes.width, align: attributes.align },
                        [
                            createBlock( 'cinderwell/column', {}, [ body( { bodyContent: attributes.leftContent, buttons: attributes.leftButtons } ) ] ),
                            createBlock( 'cinderwell/column', {}, [ body( { bodyContent: attributes.rightContent, buttons: attributes.rightButtons } ) ] ),
                        ]
                    );
                    return [ ...header, columns, ...footnote ];
                },
            },
        ],
    },
