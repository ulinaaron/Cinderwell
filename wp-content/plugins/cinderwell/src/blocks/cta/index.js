...
registerBlockType( metadata.name, {
    edit: ( { attributes, setAttributes } ) => {
        const blockProps = useBlockProps( getBackgroundImageProps( {
            className: `cinderwell-cta cinderwell-cta--bg-${ attributes.background } cinderwell-cta--align-${ attributes.alignment }${ getTypographyClassName( attributes ) }`,
        }, attributes ) );
            ...
            <>
                ...
                <div { ...blockProps }>
                    <div className="cinderwell-cta__inner" style={ { maxWidth: `var(--cw-width-${ attributes.width })` } }>
                        ...
                        { attributes.showBody && <RichText tagName="p" identifier="bodyContent" className={ `cinderwell-cta__body${ getTextStyleClassName( attributes, 'body' ) }` } value={ attributes.bodyContent } onChange={ ( v ) => setAttributes( { bodyContent: v } ) } placeholder={ __( 'Body text…', 'cinderwell' ) } allowedFormats={ ALLOWED_BODY_FORMATS } /> }
    ...
    save: ( { attributes } ) => {
        const blockProps = useBlockProps.save( getBackgroundImageProps( { className: `cinderwell-cta cinderwell-cta--bg-${ attributes.background } cinderwell-cta--align-${ attributes.alignment }${ getTypographyClassName( attributes ) }` }, attributes ) );
        return (
            <div { ...blockProps }>
                <div className="cinderwell-cta__inner" style={ { maxWidth: `var(--cw-width-${ attributes.width })` } }>
                    ...
                    { attributes.showBody && attributes.bodyContent && <RichText.Content tagName="p" className={ `cinderwell-cta__body${ getTextStyleClassName( attributes, 'body' ) }` } value={ attributes.bodyContent } /> }
